...
SUPPORTED_AGENT_PROVIDERS = {"azure_openai", "azure", "openai", "anthropic"}


def _agent_runtime_is_available() -> bool:
    try:
...
def _build_langfuse_invoke_config(
    *,
    state: StatePayload,
    ...
    return {"callbacks": callbacks, "metadata": langfuse_metadata}


def _run_agent_llm_node(
    state: StatePayload,
    *,
    config: JSONMapping,
    execution_context: Optional[ExecutionContext],
