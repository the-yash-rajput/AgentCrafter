...
export const TopBar = ({
  ...
  const displayName = agent?.name || `Agent ${agent?.id ?? ''}`.trim()

  return (
    <div
      className="flex flex-col gap-2 px-3 py-2.5 border-b shrink-0 lg:flex-row lg:items-center lg:gap-3 lg:px-4"
    ...
    >
      <div className="flex min-w-0 items-center gap-3 lg:flex-1">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2">
          <div className="w-2 h-2 rounded-full shrink-0" style={{ background: statusColors[agent?.status] || '#64748b' }} />
          <div
            className="min-w-0 flex-1 text-sm font-semibold text-white truncate"
            style={{ maxWidth: '240px' }}
            title={displayName}
          >
            {displayName}
          </div>
          {isDirty && <span className="shrink-0 text-xs" style={{ color: 'var(--text-muted)' }}>●</span>}
        </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 lg:flex-wrap lg:justify-end lg:overflow-visible lg:pb-0">
        {validation && (
          <div
            className="shrink-0 whitespace-nowrap flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-mono"
            style={{
        ...
        <button
          ...
          className="shrink-0 whitespace-nowrap flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors hover:opacity-80"
          style={{ background: 'var(--surface2)', border: '1px solid var(--border2)', color: 'var(--text-dim)' }}
        >
          <Database size={12} />
          <span className="lg:hidden">Schema</span>
          <span className="hidden lg:inline">State Schema</span>
        </button>
        ...
        <button
          ...
          className="shrink-0 whitespace-nowrap flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors hover:opacity-80 disabled:opacity-50"
        ...
        <button
          onClick={handleRun}
          disabled={launching}
          className="shrink-0 whitespace-nowrap flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-xs font-semibold transition-all hover:opacity-90 disabled:opacity-60"
